...
- (void)onCall:(LinphoneCall*)call StateChanged:(LinphoneCallState)state withMessage:(const char *)message {
    ...
    // 回调message参数
    NSDictionary *msgDic = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithUTF8String:message], @"message", [[UCSIPCCManager instance] getRemoteAddress], @"remote_address", nil];
    
    
    if ([UCSIPCCManager instance].delegate) {
        ...
        switch (state) {
            ...
            default:
                [UCSIPCCSDKLog saveDemoLogInfo:@"通话状态更新" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, msgDic]];
                break;
...
- (void)onRegister:(LinphoneCore *)lc cfg:(LinphoneProxyConfig*) cfg state:(LinphoneRegistrationState) state message:(const char*) message {
    ...
    [UCSIPCCSDKLog saveDemoLogInfo:@"登陆状态更新" withDetail:[NSString stringWithFormat:@"state:%d,\nmessage:%@", state, [NSString stringWithUTF8String:message]]];
