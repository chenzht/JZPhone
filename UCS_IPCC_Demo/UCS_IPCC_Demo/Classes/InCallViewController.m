...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // Update on show
    UCSCall* call = linphone_core_get_current_call([LinphoneManager getLc]);
    uCSCallState state = (call != NULL)?linphone_call_get_state(call): 0;
...
- (void)callUpdate:(UCSCall *)call state:(UCSCallState)state animated:(BOOL)animated {
...
- (void)hideSpinnerIndicator: (UCSCall*)call {
    videoWaitingForFirstImage.hidden = TRUE;
}

static void hideSpinner(UCSCall* call, void* user_data) {
...
- (void)displayAskToEnableVideoCall:(UCSCall*) call {
